...
<template>
  <el-dialog :title="showTitle" :visible="showDialog" @close="btnCancel">
    <el-form ref="deptFrom" :model="formData" label-width="120px" :rules="rules">
...
<script>
import { getDepartments, addDeparmrnts, getDepartDetail, updateDepartments } from '@/api/departments'
      ...
      this.$refs.deptFrom.validate(async isOK => {
        if (isOK) {
          ...
          if (this.formData.id) {
            ...
            await addDeparmrnts({ ...this.formData, pid: this.treeNode.id }) // 调用新增接口 添加父部门的id
          }
